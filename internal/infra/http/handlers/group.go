...
func (h *GroupHandler) GetGroupInfoFromLink(w http.ResponseWriter, r *http.Request) {
	...
	if inviteLink == "" {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		if encErr := json.NewEncoder(w).Encode(common.NewErrorResponse("Invite link is required")); encErr != nil {
			h.logger.Error("Failed to encode error response: " + encErr.Error())
		}
		return
	}
...
func (h *GroupHandler) GetGroupInfoFromInvite(w http.ResponseWriter, r *http.Request) {
	...
	if req.GroupJID == "" || req.Code == "" {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		if encErr := json.NewEncoder(w).Encode(common.NewErrorResponse("Group JID and code are required")); encErr != nil {
			h.logger.Error("Failed to encode error response: " + encErr.Error())
		}
		return
	}
...
func (h *GroupHandler) handleGroupActionWithJID(
	...
	if groupJID == "" {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		if encErr := json.NewEncoder(w).Encode(common.NewErrorResponse("Group JID is required in request body")); encErr != nil {
			h.logger.Error("Failed to encode error response: " + encErr.Error())
		}
		return
	}
...
func (h *GroupHandler) GetGroupInfo(w http.ResponseWriter, r *http.Request) {
	...
	if groupJID == "" {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		if encErr := json.NewEncoder(w).Encode(common.NewErrorResponse("Group JID is required as query parameter: ?groupJid=...")); encErr != nil {
			h.logger.Error("Failed to encode error response: " + encErr.Error())
		}
		return
	}
